...
bool VulkanSwapchain::Init(VulkanPhysicalDevice *physicalDevice,
                           VkDevice device, VkSurfaceKHR surface,
                           uint32_t width, uint32_t height)
{
    if (nullptr == physicalDevice || device == VK_NULL_HANDLE) {
        PSG::PrintError("创建交换链失败 物理或者逻辑设备为空!");
        return false;
    }

    // 查询物理设备 + Surface 对 Swapchain 的支持情况
    SupportDetails support = querySupport(physicalDevice->Get(), surface);
    ...
    uint32_t graphicsQueueFamily = physicalDevice->GetGraphicsQueueFamily();
    uint32_t presentQueueFamily = physicalDevice->GetPresentQueueFamily();
