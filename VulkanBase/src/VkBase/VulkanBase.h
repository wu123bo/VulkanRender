...
private:
    ...
    // 切换纹理
    void updateTextureIfNeeded(uint32_t currentImage);
...
private:
    ...
    std::vector<VulkanUniformBuffer> _uniformLightBuffer;
    ...
    std::vector<VulkanTexture> _textures; // 纹理对象
...
private:
    ...
    const std::vector<VerCorTexNor> _vertices = {
        // Front (+Z)
