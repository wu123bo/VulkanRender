...
struct LightInfo
{
    alignas(16) PTF_3D lightPos;
    alignas(16) PTF_3D lightColor;
    alignas(16) PTF_3D viewPos;
};
...
struct VerCorTexNor
{
    ...
    static VkVertexInputBindingDescription getBindingDescription()
    {
        ...
        bindingDescription.binding = 0;
        // 指定从一个条目到下一个条目的字节数
        bindingDescription.stride = sizeof(VerCorTexNor);
        // 在每个顶点之后移动到下一个数据条目
        bindingDescription.inputRate = VK_VERTEX_INPUT_RATE_VERTEX;
    ...
    getAttributeDescriptions()
    {
        ...
        attributeDescriptions[0].format = VK_FORMAT_R32G32B32_SFLOAT;
        // 定自每个顶点数据的开始读取的字节数
        attributeDescriptions[0].offset = offsetof(VerCorTexNor, pos);
        ...
        attributeDescriptions[1].location = 1;
        attributeDescriptions[1].format = VK_FORMAT_R32G32B32_SFLOAT;
        attributeDescriptions[1].offset = offsetof(VerCorTexNor, color);

        // 颜色属性相同设置
        attributeDescriptions[2].binding = 0;
        attributeDescriptions[2].location = 2;
        attributeDescriptions[2].format = VK_FORMAT_R32G32_SFLOAT;
        attributeDescriptions[2].offset = offsetof(VerCorTexNor, texCoord);

        // 颜色属性相同设置
        attributeDescriptions[3].binding = 0;
        attributeDescriptions[3].location = 3;
        attributeDescriptions[3].format = VK_FORMAT_R32G32B32_SFLOAT;
        attributeDescriptions[3].offset = offsetof(VerCorTexNor, normal);
